...
export default class BaseMenu extends PureComponent {
  ...
  getSubMenuOrItem = item => {
    ...
    // TODO 判断登入用户，根据用户选择菜单渲染
    // 获取相应用户的菜单显示
    const hideMenu1=['统一身份','对外接口'];
    const hideMenu2=['统一身份','便民服务','对外接口'];
    if(sessionStorage.getItem('sys-role') === 'role2'){
      if(hideMenu1.indexOf(formatMessage({ id: item.locale })) !== -1){
        return null;
      }
    }else if(sessionStorage.getItem('sys-role') === 'role3'){
      if(hideMenu2.indexOf(formatMessage({ id: item.locale })) !== -1){
        return null;
      }
    }
    /*
    if(formatMessage({ id: item.locale }) === '对外接口')
      return null;
    */
