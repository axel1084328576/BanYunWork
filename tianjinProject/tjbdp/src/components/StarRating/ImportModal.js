...
@connect(({starRating,loading}) => {
  ...
  return{
    pageSize,page,
    loading:loading.effects['starRating/Upload'],
  }
})
  ...
  ImportSubmit=(e)=>{
    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
        ...
        if(document.getElementById("importExcel27").files[0]!=undefined){
          formdata.append('file',document.getElementById("importExcel27").files[0]);
        }
        for(let key in data){
          formdata.append(key,data[key]);
        }
        _this.props.dispatch({
          type:"starRating/Upload",
          payload: formdata,
          callback:(value)=>{
            if(value){
              _this.props.dispatch({
