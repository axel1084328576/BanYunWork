...
@connect(({expressBox,loading}) => {
  ...
  return{
    pageSize,page,
    loading:loading.effects['expressBox/Upload'],
  }
})
  ...
  ImportSubmit=(e)=>{
    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
        ...
        if(document.getElementById("importExcel24").files[0]!=undefined){
          formdata.append('file',document.getElementById("importExcel24").files[0]);
        }
        for(let key in data){
          formdata.append(key,data[key]);
        }
        _this.props.dispatch({
          type:"expressBox/Upload",
          payload: formdata,
          callback:(value)=>{
            if(value){
              _this.props.dispatch({
