...
@connect(({networkManage,loading}) => {
  ...
  return{
    pageSize,page,
    loading:loading.effects['networkManage/Upload'],
  }
})
  ...
  ImportSubmit=(e)=>{
    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
        ...
        if(document.getElementById("importExcel21").files[0]!=undefined){
          formdata.append('file',document.getElementById("importExcel21").files[0]);
        }
        for(let key in data){
          formdata.append(key,data[key]);
        }
        _this.props.dispatch({
          type:"networkManage/Upload",
          payload: formdata,
          callback:(value)=>{
            if(value){
              _this.props.dispatch({
