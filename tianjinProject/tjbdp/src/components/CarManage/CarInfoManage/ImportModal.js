...
@connect(({carInfoManage,loading}) => {
  ...
  return{
    pageSize,page,
    loading:loading.effects['carInfoManage/Upload'],
  }
})
  ...
  ImportSubmit=(e)=>{
    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
        ...
        if(document.getElementById("importExcel28").files[0]!=undefined){
          formdata.append('file',document.getElementById("importExcel28").files[0]);
        }
        for(let key in data){
          formdata.append(key,data[key]);
        }
        _this.props.dispatch({
          type:"carInfoManage/Upload",
          payload: formdata,
          callback:(value)=>{
            if(value){
              _this.props.dispatch({
