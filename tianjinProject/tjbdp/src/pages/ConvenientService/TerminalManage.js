...
import AddModal from '../../components/ConvenientService/TerminalManage/AddModal';
import EditModal from '../../components/ConvenientService/TerminalManage/EditModal';
import ImportModal from '../../components/ConvenientService/TerminalManage/ImportModal';

@connect(({ terminalManage,getPagePage,loading }) => {
  const {list,pageSize,total,controlList,page,}=terminalManage;
  const {pagePath}=getPagePage;
  return{
    list,pageSize,total,controlList,page,pagePath,
    loading: loading.effects['terminalManage/List'],
  }
})


@Form.create()
export default class TerminalManage extends Component{
  state={
    addVisible:false,
    editVisible:false,
    importVisible:false,
    ...
    columns:[
      ...
      },
      {
        ...
        render: (text, record) => {
          return (
            <span>
            {this.props.controlList.md_edit? <a href="javascript:" onClick={() => {
              this.showEditModal(record);
            }}>修改</a>:null}
              {this.props.controlList.md_edit && this.props.controlList.md_del?<Divider type="vertical" />:null}
              {
                this.props.controlList.md_del?(this.props.list.length >= 1
                  ? (
                    <Popconfirm
                      title="是否要删除选中的用户信息?"
          ...
          <Row>
            ...
            <Col xs={{ span: 12 }} sm={{ span: 12 }} lg={{offset:6,span:6}}>
              <Row type="flex" justify="end">
                {this.props.controlList.md_imp?<span className={styles.unSpan1}>
                   <Button onClick={this.handleImportVisible}><Icon type="upload" /> 导入</Button>
                </span>:null}
                <span className={styles.unSpan1}>
                  {this.props.controlList.md_del?(selectedRowKeys.length>0?<Popconfirm
                    okText="确定"
                    cancelText="取消"
                    title="是否要删除选中的用户信息?"
                    placement="bottomRight"
                  ...
                  {this.props.controlList.md_add?<Button style={{ marginLeft: 10 }} type="primary" icon="plus" onClick={this.showAddModal}>添加</Button>:null}
                </span>
              </Row>
            </Col>
          </Row>
